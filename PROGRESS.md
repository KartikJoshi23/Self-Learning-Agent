...
|---|---|---|---|---|---|
| 2026-09-12 | Master laptop | Master | 4 (site) | **Visual overhaul after the Master rejected the first look** (flat cards, one colour, no hover): a fixed colour field behind everything (static radial gradients + three drifting orbs), real glass — gradient borders via masked pseudo-elements, inner highlight, accent-tinted shadows, cursor-following glow and lift on hover — an accent per page (ochre / blue / violet / aqua / amber / orange) driving eyebrow dots, borders, chips, sliders, buttons and the nav pill, gradient page titles, glass prev/next cards. Series colours re-validated against the new surface. Fixed a real bug found on the way: a transparent WebGL canvas came back blank after leaving the viewport, so the colour field is now painted inside the opaque scene. Re-audited every route (16/16 clean), Lighthouse home 80 / content 94–98 / a11y 100. | *(this commit)* |
